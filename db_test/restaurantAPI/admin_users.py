...
@admin_users_blueprint.route('/adminUsers/create', methods=["POST"])
@swag_from({
    'tags': ['Admin Users'],
...
@admin_users_blueprint.route('/adminUsers/stripeKey', methods=["PUT"])
@swag_from({
    'tags': ['AdminUsers'],
    'description': 'Update the Stripe API key for the authenticated admin user.',
    'security': [{'BearerAuth': []}],
    'parameters': [
        {
            'name': 'body',
            ...
            'schema': {
                ...
                'properties': {
                    'stripeKey': {
                        'type': 'string',
                        'description': 'New Stripe API key to set for the admin user.',
...
def update_stripe_key():
    ...
    insert_db("UPDATE adminuser SET stripeKey = %s WHERE id = %s", args=(stripekey, adminID))
    return jsonify({"message": "Admin user updated successfully"}), 200
